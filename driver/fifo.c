...
static unsigned int buffer[FIFO_BUF_SIZE];
static unsigned int in_index, out_index;


/* Write n data to buffer */
int WriteBuf(unsigned int *data, unsigned int n)
{
	unsigned int i = 0;
...
int ReadBuf(unsigned int *out_buf, unsigned int n)
{
	unsigned int i = 0;
	unsigned int data_count = 0;
...
void LookBuf(unsigned int *out_buf)
{
	unsigned int i = 0;
